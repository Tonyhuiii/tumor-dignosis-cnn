...
m=len(label_list)
label_list.sort(key=lambda x:(int(x.split('.')[0].split('_')[1]),int(x.split('.')[0].split('_')[2]) if len(x.split('.')[0].split('_'))==3 else 0))
# label_list.sort(key=lambda x:int(x.split('.')[0][-3:]))

def crop_tumor(root,slide,path,level):
    savepath1=path+'/original'
    ...
    for a in root.iter('Annotations'):
        for b in a.iter('Annotation'):
            ...
            for c in b.iter('Coordinates'):
                ...
                x0=int(x1)
                y0=int(y1)
                w=int((x2-x0)/downsample)+1
                h=int((y2-y0)/downsample)+1
                img1=slide.read_region((x0,y0),level,(w,h)).convert('RGB')
                if (group!='9'):
                    img1.save(savepath1+'/{}_{}.jpg'.format(k,group))
                else:
                    img=slide.read_region((x0,y0),0,(int(x2-x0)+1,int(y2-y0)+1)).convert('RGB')
                    # print(img.size)
                    img1.save(path+'/slide.jpg')
                    print(img1.size)
                    x3=x0
                    y3=y0
                    print(x3)
    for i in region:
        for j in range (len(region[i])):
            region[i][j][0]=(region[i][j][0]-x3)/downsample
            region[i][j][1]=(region[i][j][1]-y3)/downsample 
        # print(region[i][j][0],region[i][j][1])  
 
    return region, img, img1.size
...
level=5
downsample=2**level
print('level:{} downsample:{}'.format(level,downsample))
for i in range(m):
    xml_path= label_path+'/'+ label_list[i]
    mode=label_list[i].split('.')[0].split('_')[0]
    if(mode=='test'):
        svs_name=label_list[i].split('.')[0][:8]
    elif (mode=='tumor'):
        svs_name=label_list[i].split('.')[0][:9]  
    elif (mode=='normal'):
        svs_name=label_list[i].split('.')[0][:10]     
    print(svs_name) 
    svs_path= slide_path+'/'+ svs_name+'.tif'
    slide = openslide.open_slide(svs_path)
    tree = ET.parse(xml_path)
    root = tree.getroot()
    result_path = opt.dataroot+'/results'
    path=result_path + '/detail/' + label_list[i].split('.')[0]
    print(label_list[i].split('.')[0])
    region,slide_image, mask_size=crop_tumor(root,slide,path,level) 
    ###generate mask
    path1=path+'/original'
    slide_resize=path+'/slide.jpg'
    img_list=os.listdir(path1)
    img_list.sort(key=lambda x: int(x.split('.')[0][:-2]))
    # print(img_list)
    image = Image.open(slide_resize)
    image = np.asarray(image)
    image = cv2.cvtColor(image,cv2.COLOR_RGB2BGR)  
    im = np.zeros(image.shape[:2], dtype="uint8")
    path0=  result_path+'/mask'   
    if not os.path.exists(path0):
        os.makedirs(path0)
    mask_path=path0+'/'+label_list[i].split('.')[0]+'.jpg'
    for j in range(len(img_list)):
        class_name=img_list[j].split('.')[0][-1]
        img_path = path1 + '/' + img_list[j]
        if (class_name!='2'):
            r1 = np.array(region[j], dtype=np.int32)
            r2 = np.expand_dims(np.asarray(r1), axis=0)
            #把所有的点画出来
            cv2.polylines(im, r2, 1, 255)
            #把所有点连接起来，形成封闭区域
            cv2.fillPoly(im, r2, 255)
        elif (class_name=='2'):
            r1 = np.array(region[j], dtype=np.int32)
            r2 = np.expand_dims(np.asarray(r1), axis=0)
            #把所有的点画出来
            cv2.polylines(im, r2, 1, 0)
            #把所有点连接起来，形成封闭区域
            cv2.fillPoly(im, r2, 0)        

    cv2.imwrite(mask_path,im)
    # masked = cv2.bitwise_and(image, image, mask=im)
    # cv2.imwrite(path2+'/tumor_masked.jpg',masked)
    ...
    # print('draw heatmap1')
    # cv2.imwrite(path4+'/1.jpg', np.uint8(img1))
